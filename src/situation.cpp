...
void SituationModule::update_stand_points(WorldModel &wm) {
    ...
    if (attack) {
        ...
        // 防守态：助攻/中场站位。
        //   威胁分级（docs/13 方案 C）：威胁 <0.6（球在中场/对方半场）→ 前压 30cm
        //     留反击支点（断球瞬间前场立刻有人接应，治真实 9/2 vs demo 断球后前场真空）；
        //     威胁 >=0.6（球在我们半场/门前）→ 随球回撤保纵深。
        //   随球回撤：旧 x=110 固定中线 → 球深进我方三区(如 x=180)时后卫仍停中线，
        //     纵深空档 ~70cm，对方持球者一路带到门前无人逼抢（复盘 0:2 失球根因）；
        //     改为 line_x 随球回撤，最多退到罚球区前缘外（末尾「己方禁区纪律」夹到 gx+85）。
