...
namespace {
// ============================================================
// docs/15 P0：激进进攻参数 + 避障移动 helper
// ============================================================
// 射门机会质量分档（shoot.cpp plan_shoot 输出 quality ∈[0,1]，仅 70~110 动态区生效）：
//   quality ≥ 立即射阈值 → 直线推射。阈值 = run_active 局部变量 shoot_now_q：
//     正常 0.35 / 领先≥2 时 0.55（P1 保守化）。A/B 记录（sim scripted, 50 场×seed1/2）：
//     · 全改动版（70 内也卡 quality 闸门 + 中档带球 + 追球全图 route）净 4.4；
//     · 70 内恢复无条件射 + 追球直线 → 净 5.7（≈基线 5.8）；
//     · + 追球 2r 邻域 route + 0.35 → 净 5.5/5.1（差 0.3~0.4，在 50 场噪音 ±0.5 内，
//       源头：动态远射闸门在弱脚本 GK 前少射的边际进球）；
//     · 0.25 vs 0.35 无显著差 → 取 0.35 保留"远距小开口不硬来"档位语义
//       （quality≈0.5·open/18+0.3·(110-d)/110，d=100 时约 open≥13° 才放行）。
//   quality < 阈值 → 落传球/围困/正常带球（带球阶段维持基线行为）。
// [A/B 记录] "中档带球逼近"分支(0.25~0.45 提前推穿) 是净胜 6.9→4.4 的成因之一，已移除；
//   追球避障用 2r 邻域裁剪图（全圆 62 顶点图每帧 ≈0.3ms，sim 50 场 13.6s→38.6s 不可用）。
constexpr double kRouteInflate = 10.0;   // 对方机器人避障半径（本体 6 + 净空 4）
constexpr double kLegacyShootRange = 70.0;  // 旧无条件射程（docs/15：70 内不加闸门）

// 避障移动（docs/15 P0-4）：从 (r.x,r.y) 向 (tx,ty)，对方 5 机器人作圆盘障碍。
...
void run_active(WorldModel &wm, int id) {
    ...
    // —— 射门（docs/15 P0-3/4：机会质量闸门 + 直线推穿执行体不变）——
    // 历史教训（docs/03 R17-18）：70cm 一刀切放宽 → 真机 0:3；
    // 分档（A/B 校准后）：
    //   · ≤70cm 旧射程：无条件立即射（sim 校验过的进球主力区，不加闸门防 A/B 退步）；
    //   · 70~110cm 动态激进区：plan_shoot 已加「净开口≥8° 且 路线通」闸门，
    //     此处再要求 quality≥kShootNowQ 才立即射（远射只在开口真大时放行）。
    //   P1（docs/15 §3.2）：领先 ≥2 阈值 0.35→0.55——领先后少冒险远射，防被扑反击。
    double shoot_now_q = wm.lead_by_two ? 0.55 : 0.35;
    ShootPlan sp = plan_shoot(wm, id);
    if (sp.viable &&
        (sp.shot_dist <= kLegacyShootRange || sp.quality >= shoot_now_q)) {
        double bx = wm.ball.x, by = wm.ball.y;
    ...
    if (db < 15.0 && db < opp_d) {
        ...
        for (int i = 0; i < PLAYERS_PER_SIDE; ++i) {
            double d = dist(wm.opp[i].x, wm.opp[i].y, wm.ball.x, wm.ball.y);
            // P1（docs/15 §3.2）：领先 ≥2 围困更敏感（半径 25→22、贴身 12→15），
            //   领先后尽早回传/带离，不冒险在包围中带球。
            double swarm_r = wm.lead_by_two ? 22.0 : 25.0;
            if (d < swarm_r) swarm++;
            if (d < opp_near) { opp_near = d; near_i = i; }
        }
        if (swarm >= 2 || opp_near < (wm.lead_by_two ? 15.0 : 12.0)) {
        ...
        // [A/B 校准] 方向维持旧"两选一门柱口"（离最近防守者远的一侧）：带球行为
        //   与基线一致（sim A/B 净胜 6.9 基准不因带球变化回退）；动态区远射机会
        //   已在射门分支用 plan_shoot 连续开口放行，带球阶段不再二次干预。
        ...
        // docs/15 P0-4：追球避障——直线被对方挡才绕行（2r 邻域裁剪后规划开销
        //   ≈ 全图的 1/10，sim 50 场 <20s）；decel=true 保留接近减速防冲过头。
        move_avoiding(wm, r, id, chased.x, chased.y, /*decel=*/true);
    }
}
