...
void position(RobotState &r, double tx, double ty) {
    ...
    // 接近目标最后 12cm：若朝向偏差还大，先减速转向对准——
    //   否则高速冲到推球点/接应点附近还边转边铲，推球方向被带偏
    //   （真实平台实测：带球射门系统性偏下，球 y 90→65 偏出门柱，主罚助跑 175cm 太长）。
    //   对准后(|te|≤30°)恢复全速直线推，射门/传球力度不受影响。
    if (de < 12.0 && std::fabs(te) > 30.0) {
        drive *= 0.25;
    }
