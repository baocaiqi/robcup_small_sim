...
// ============================================================
// docs/15 P0-1：圆盘碰撞原语单测（segment_clear_of_circles）
// ============================================================
...
// ============================================================
static int test_active_aggressive() {
    ...
    // A: 球 (80,90)（dgoal=80 动态区）、GK 站门口中央 → 远射档
    wm.ball.x = 80; wm.ball.y = 90;
    wm.opp[0].x = 3; wm.opp[0].y = 90;
    wm.home[1].x = 76; wm.home[1].y = 90; wm.home[1].rot = 180;
    run_active(wm, 1);
    if (!std::isfinite(wm.home[1].vl) || !std::isfinite(wm.home[1].vr)) { printf("FAIL: 远射档 NaN\n"); return 1; }
    if (fmax(fabs(wm.home[1].vl), fabs(wm.home[1].vr)) > 300.0) { printf("FAIL: 远射档轮速越界\n"); return 1; }

    // B: GK 贴身封死（球前 6cm）→ 开口<8° 拒绝远射，落围困带离（不原地死等）
    wm.opp[0].x = 74; wm.opp[0].y = 90;
    wm.home[1].x = 76; wm.home[1].y = 90; wm.home[1].rot = 180;
    run_active(wm, 1);
    if (!std::isfinite(wm.home[1].vl) || !std::isfinite(wm.home[1].vr)) { printf("FAIL: 封死档 NaN\n"); return 1; }
    if (fmax(fabs(wm.home[1].vl), fabs(wm.home[1].vr)) > 300.0) { printf("FAIL: 封死档轮速越界\n"); return 1; }

    // C: 追球路径被防守者挡 → route 绕行（沿路径采样不穿盘，目标可达）
    wm.ball.x = 30; wm.ball.y = 90;      // 球在我方半场深处，ACTIVE 去追
    wm.opp[0].x = 200; wm.opp[0].y = 90; // GK 复位
    wm.opp[1].x = 70; wm.opp[1].y = 90;  // 防守者站在追球直线上
    wm.home[1].x = 110; wm.home[1].y = 90; wm.home[1].rot = 180;
    run_active(wm, 1);
    if (!std::isfinite(wm.home[1].vl) || !std::isfinite(wm.home[1].vr)) { printf("FAIL: 追球避障 NaN\n"); return 1; }
    if (fmax(fabs(wm.home[1].vl), fabs(wm.home[1].vr)) > 300.0) { printf("FAIL: 追球避障轮速越界\n"); return 1; }
    printf("active aggressive: OK (远射档/封死拒绝/追球避障 无崩溃无NaN轮速有界)\n");
    return 0;
}

// ============================================================
// docs/15 P1：比分自数
//   蓝视角：守 x=220、攻 x=0。判据 A = PlayOn→PlaceKick 事件（进球后失球方开球，
//   docs/06 L347 实测）；判据 B = 球完全越过门线且 y∈[70,110]（越线兜底）。
// ============================================================
static int test_score_counting() {
    TeamContext ctx{true};
    WorldModel wm;
    wm.ctx = ctx;
    Strategy strat;
    ...
    // —— 判据 B：越线兜底 ——
    // ① 球完全越过对方门线(0) → 我方得分 + 锁存
    wm.ball.x = -3; wm.ball.y = 90;
    strat.update_score(wm);
    if (wm.score_us != 1 || !wm.goal_line_armed) {
        printf("FAIL: 越线应计分并锁存 us=%d armed=%d\n", wm.score_us, wm.goal_line_armed); return 1;
    }
    // ② 同一次越线（球停门内）不重复计
    strat.update_score(wm);
    if (wm.score_us != 1) { printf("FAIL: 锁存期内重复计分 us=%d\n", wm.score_us); return 1; }
    // ③ 球弹回场内（PlayOn 默认）→ 解锁
    wm.ball.x = 40;
    strat.update_score(wm);
    if (wm.goal_line_armed || wm.score_us != 1) { printf("FAIL: 离门应解锁\n"); return 1; }
    // ④ 再次越线 → 计第 2 分
    wm.ball.x = -5; wm.ball.y = 105;
    strat.update_score(wm);
    if (wm.score_us != 2) { printf("FAIL: 第二次越线应计 2 us=%d\n", wm.score_us); return 1; }
    // ⑤ 我方门线(220)越线 → 对方得分
    wm.ball.x = 40; strat.update_score(wm);        // 解锁
    wm.ball.x = 223; wm.ball.y = 90;
    strat.update_score(wm);
    if (wm.score_them != 1) { printf("FAIL: 我方门线越线应计对方分 them=%d\n", wm.score_them); return 1; }
    // ⑥ 门柱外（y<70）越线不计数
    wm.ball.x = 40; strat.update_score(wm);        // 解锁
    wm.ball.x = -3; wm.ball.y = 60;
    strat.update_score(wm);
    if (wm.score_us != 2) { printf("FAIL: 门柱外不应计分 us=%d\n", wm.score_us); return 1; }

    // —— 判据 A：PlayOn → PlaceKick 事件（进球后失球方开球）——
    // ⑦ 黄开球 = 黄刚失球 = 我们(蓝)得分
    wm.ball.x = 40; strat.update_score(wm);        // 解锁
    wm.ball_seen_moving = true;                    // 已见过活球（开局守卫放行）
    wm.game_state_last = PM_PlayOn;
    wm.game_state = PM_PlaceKick_Yellow;
    wm.ball.x = 110; wm.ball.y = 90;
    strat.update_score(wm);
    if (wm.score_us != 3) { printf("FAIL: PlaceKick_Yellow 应计我方分 us=%d\n", wm.score_us); return 1; }
    // ⑧ 同事件只计一次（下一帧 gs_last 已是 PlaceKick）
    wm.game_state_last = PM_PlaceKick_Yellow;
    strat.update_score(wm);
    if (wm.score_us != 3) { printf("FAIL: PlaceKick 持续帧重复计分 us=%d\n", wm.score_us); return 1; }
    // ⑨ 比赛恢复解锁 → 蓝开球 = 蓝刚失球 = 对方得分
    wm.game_state_last = PM_PlaceKick_Yellow;
    wm.game_state = PM_PlayOn; wm.ball.vx = 5.0;
    strat.update_score(wm);                        // PlayOn 且离门 → 解锁
    wm.ball.vx = 0; wm.ball_seen_moving = true;
    wm.game_state_last = PM_PlayOn;
    wm.game_state = PM_PlaceKick_Blue;
    strat.update_score(wm);
    if (wm.score_them != 2) { printf("FAIL: PlaceKick_Blue 应计对方分 them=%d\n", wm.score_them); return 1; }
    // ⑩ 开局守卫：没见过活球时的 PlaceKick 不计分（开局开球非进球）
    {
        WorldModel wm2;
        wm2.ctx = ctx;
        wm2.game_state_last = PM_PlayOn;
        wm2.game_state = PM_PlaceKick_Blue;
        wm2.ball.x = 110; wm2.ball.y = 90;
        strat.update_score(wm2);
        if (wm2.score_us != 0 || wm2.score_them != 0) {
            printf("FAIL: 开局 PlaceKick 不应计分 us=%d them=%d\n", wm2.score_us, wm2.score_them); return 1;
        }
    }
    // ⑪ 非 PlayOn → PlaceKick 转换不计（中场/自由球切来的开球态不算进球）
    wm.game_state_last = PM_PlaceKick_Blue; wm.game_state = PM_PlayOn;
    wm.ball.x = 40; strat.update_score(wm);        // 解锁（⑨ 计分后 armed）
    wm.game_state_last = PM_GoalKick_Blue;
    wm.game_state = PM_PlaceKick_Yellow;
    strat.update_score(wm);
    if (wm.score_us != 3) { printf("FAIL: 非 PlayOn 转换不应计分 us=%d\n", wm.score_us); return 1; }
    // ⑫ lead_by_two：3:2 不触发；4:2 触发
    wm.game_state_last = PM_PlaceKick_Yellow; wm.game_state = PM_PlayOn;
    wm.ball.x = 40; strat.update_score(wm);        // 解锁
    if (wm.lead_by_two) { printf("FAIL: 3:2 不应触发领先防守\n"); return 1; }
    wm.ball_seen_moving = true;
    wm.game_state_last = PM_PlayOn; wm.game_state = PM_PlaceKick_Yellow;
    wm.ball.x = 110;
    strat.update_score(wm);                        // → 4:2
    if (!wm.lead_by_two) { printf("FAIL: 4:2 应触发领先防守\n"); return 1; }

    printf("score counting: OK (越线计分/锁存防重/PlaceKick事件/开局守卫/领先触发)\n");
    return 0;
}

// ============================================================
// docs/15 P1：领先 ≥2 冒险度保守（动态区中等质量机会：正常立即射，领先不射）
//   行为代理断言：两次 run_active 决策输出不同（射门档 vs 保守分支），且都有界。
// ============================================================
static int test_lead_defense() {
    TeamContext ctx{true};
    WorldModel wm;
    wm.ctx = ctx;
    for (int i = 0; i < 5; ++i) { wm.opp[i].x = 200; wm.opp[i].y = 30 + i * 20; }
    ...
    // GK 贴上门角出击：门中心方向敞开 → 单段开口 18.5°、quality≈0.54 ∈ [0.35,0.55)
    //   （GK 站门中央时单段开口恒 <8°，不满足"正常该射"前置，故用偏角场景）
    wm.opp[0].x = 5; wm.opp[0].y = 110;

    // 前置：机会质量落在 [0.35, 0.55)——正常该射、领先不该射的判别区
    wm.home[1].x = 96; wm.home[1].y = 90; wm.home[1].rot = 180;
    ShootPlan sp = plan_shoot(wm, 1);
    if (!sp.viable || sp.quality < 0.35 || sp.quality >= 0.55) {
        printf("FAIL: 前置质量应在 [0.35,0.55) viable=%d q=%.2f\n", sp.viable, sp.quality); return 1;
    }
    // 正常（lead=false）：进入射门档
    wm.lead_by_two = false;
    wm.home[1].x = 96; wm.home[1].y = 90; wm.home[1].rot = 180;
    wm.home[1].vl = wm.home[1].vr = 0;
    run_active(wm, 1);
    double vn_l = wm.home[1].vl, vn_r = wm.home[1].vr;
    if (!std::isfinite(vn_l) || !std::isfinite(vn_r) || fmax(fabs(vn_l), fabs(vn_r)) > 300.0) {
        printf("FAIL: 正常模式轮速异常 %.1f %.1f\n", vn_l, vn_r); return 1;
    }
    // 领先（lead=true）：保守分支，决策应不同
    wm.lead_by_two = true;
    wm.home[1].x = 96; wm.home[1].y = 90; wm.home[1].rot = 180;
    wm.home[1].vl = wm.home[1].vr = 0;
    run_active(wm, 1);
    double vl_l = wm.home[1].vl, vl_r = wm.home[1].vr;
    if (!std::isfinite(vl_l) || !std::isfinite(vl_r) || fmax(fabs(vl_l), fabs(vl_r)) > 300.0) {
        printf("FAIL: 领先模式轮速异常 %.1f %.1f\n", vl_l, vl_r); return 1;
    }
    if (std::fabs(vl_l - vn_l) < 1e-6 && std::fabs(vl_r - vn_r) < 1e-6) {
        printf("FAIL: 领先应改变射门决策 (vl %.1f→%.1f)\n", vn_l, vl_l); return 1;
    }
    printf("lead defense: OK (正常射门档 vs 领先保守档 决策分离、轮速有界)\n");
    return 0;
}
...
static int test_shoot_plan() {
    ...
    wm.ctx = ctx;
    wm.ball.valid = true;
    wm.ball.x = 25; wm.ball.y = 90;      // 球在对方门前 25cm（dgoal=25 ≤110 → 可射）
    for (int i = 0; i < 5; ++i) { wm.opp[i].x = 200; wm.opp[i].y = 30 + i * 20; }  // 其余远离，别挡射门线
    wm.opp[0].x = 3;  wm.opp[0].y = 90;  // 对方守门员站门中央（离门线最近）
    {
        ...
        if (p.quality <= 0.0) { printf("FAIL: 近距大开口 quality 应>0 (%.3f)\n", p.quality); return 1; }
    }
    // GK 偏上(y=105 贴上柱) → 净开口偏下 → aim_y 应 < 90（不再是固定 74）
    wm.opp[0].y = 105;
    {
        ShootPlan p2 = plan_shoot(wm, 1);
        if (!p2.viable) { printf("FAIL: GK 偏上仍应可射 viable=%d\n", p2.viable); return 1; }
        if (p2.aim_y >= 90.0) {
            printf("FAIL: GK 偏上应瞄下侧开口 got aim=%.1f\n", p2.aim_y); return 1;
        }
    }
    printf("shoot plan: OK (可射/dir单位/推球点/GK远侧连续开口/quality>0)\n");
    return 0;
}

// ============================================================
// docs/15 P0-3：机会质量动态射程单测
// ============================================================
static int test_shoot_quality() {
    TeamContext ctx{true};
    WorldModel wm;
    wm.ctx = ctx;
    wm.ball.valid = true;
    for (int i = 0; i < 5; ++i) { wm.opp[i].x = 200; wm.opp[i].y = 30 + i * 20; }  // 默认远离
    // (a) 近距 25cm、GK 贴身封中央（遮挡 40° > 门张角一半）→ 两侧开口仍在 → 可射
    wm.ball.x = 25; wm.ball.y = 90;
    wm.opp[0].x = 3;  wm.opp[0].y = 90;
    {
        ShootPlan p = plan_shoot(wm, 1);
        if (!p.viable) { printf("FAIL: 25cm 近距两侧开口应可射\n"); return 1; }
        if (p.open_angle < 10.0) { printf("FAIL: 近距开口应>10° got %.1f\n", p.open_angle); return 1; }
    }
    // (b) 远距 100cm、GK 贴身(球前 10cm)封中央 → 遮挡角 >> 门张角 → 不可射（防 0:3 关键闸门）
    wm.ball.x = 100; wm.ball.y = 90;
    wm.opp[0].x = 90; wm.opp[0].y = 90;
    {
        ShootPlan p = plan_shoot(wm, 1);
        if (p.viable) { printf("FAIL: 远距 GK 封死不应射 viable=%d open=%.1f\n", p.viable, p.open_angle); return 1; }
    }
    // (c) 远距 100cm、GK 出击偏到门外侧 → 门张角全开 → 远射放行（动态射程 >70 的正当场景）
    wm.opp[0].x = 90; wm.opp[0].y = 60;
    {
        ShootPlan p = plan_shoot(wm, 1);
        if (!p.viable) { printf("FAIL: 远距 GK 偏外侧应放行远射\n"); return 1; }
        if (p.shot_dist < 70.0) { printf("FAIL: 用例应处于 70~110 动态射程区\n"); return 1; }
        if (p.quality < 0.25) { printf("FAIL: 全开远射 quality 应中上 (%.2f)\n", p.quality); return 1; }
    }
    // (d) 路线拦截（70~110 动态区才查线）：防守者站在开口中心线上 → lane_blocked，不硬射
    wm.ball.x = 80; wm.ball.y = 90;       // dgoal=80 ∈ 动态激进区
    wm.opp[0].x = 3;  wm.opp[0].y = 90;   // GK 站门中央（离门线最近）
    {
        ShootPlan p0 = plan_shoot(wm, 1);
        if (!p0.viable) { printf("FAIL: 前置条件应可射\n"); return 1; }
        // 在开口中心线方向 15cm 处放拦截者
        double lx = wm.ball.x + p0.dir_x * 15.0;
        double ly = wm.ball.y + p0.dir_y * 15.0;
        wm.opp[1].x = lx; wm.opp[1].y = ly;
        ShootPlan p = plan_shoot(wm, 1);
        if (p.viable) { printf("FAIL: 动态区射门线被挡应不可射 viable=%d\n", p.viable); return 1; }
    }
    // (d2) 旧射程区(≤70cm)不受线闸门约束（维持旧无条件射）：25cm 线上有人也应可射
    wm.ball.x = 25; wm.ball.y = 90;
    wm.opp[0].x = 3; wm.opp[0].y = 90;
    wm.opp[1].x = 15; wm.opp[1].y = 90;   // 站在球-门直线上
    {
        ShootPlan p = plan_shoot(wm, 1);
        if (!p.viable) { printf("FAIL: ≤70cm 维持旧无条件射 viable=%d\n", p.viable); return 1; }
    }
    // (e) quality 单调：同样 GK 布局下，球越近 quality 越高
    {
        wm.opp[1].x = 200; wm.opp[1].y = 50;    // 移走拦截者
        wm.ball.x = 25; wm.ball.y = 90;
        double q_near = plan_shoot(wm, 1).quality;
        wm.ball.x = 80; wm.ball.y = 90;
        double q_far = plan_shoot(wm, 1).quality;
        if (q_near <= q_far) { printf("FAIL: quality 应随射程缩短而升 near=%.2f far=%.2f\n", q_near, q_far); return 1; }
    }
    printf("shoot quality: OK (近距可射/远距封死拒绝/远距全开放行/路线拦截/质量单调)\n");
    return 0;
}
