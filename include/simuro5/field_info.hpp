...
// ============================================================
// 球撞边墙的实测速度系数（docs/06 第 65 轮；唯一真值来源，别在别处再写一份）
//   实测方法：118 场真机 .rlg、273 个弹墙样本，取 vy 局部极值 + 贴墙 8cm 内 +
//   只算 PlayOn 帧 + 剔除单帧位移>30cm 的平台瞬移 + 排除球门口内的进球。
//   结论（中位）：法向恢复 **0.66**、切向保持 **0.81** ⇒ **入射角 ≠ 反射角**，
//   出射线会明显往墙那边"扫"（45° 入射时出射速度只剩 74%）。
//   ⚠️ 因此**不能用理想镜面**（法向×1.0）做反弹预测：那会把落点算得离墙偏远。
//   用途：进攻借墙射门（shoot.cpp）、防守断球点/门将反弹位（defense.hpp）。
// ============================================================
inline double ball_wall_rest() { return 0.66; }   // 法向分量恢复系数
inline double ball_wall_fric() { return 0.81; }   // 切向分量保持系数
