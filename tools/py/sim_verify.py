...
def main():
    ...
    print("="*70)
    print("已确认 rlg 与官方 log 比分关系（2026-08-29 复核）：")
    print("  - 官方 log 比分格式 = (黄 : 蓝)，首位是黄队！此前把 (7:2)/(8:2) 误读为蓝队比分")
    print("  - rlg 物理越线统计（4 doubles/机器人 + 连续帧去重）与 log 方向一致：")
    print("    8/26 场 rlg 黄8蓝2 = log (8:2)；8/24 场 rlg 黄4蓝1 ≈ log (3:1)；8/29 场 rlg 黄1 = log (1:0)")
    print("  - 权威比分 = 官方 SimuroSot5.log（平台自己记分），但解析时必须按 (黄:蓝)")
    print("  - 真实基线：我们(蓝) vs demo(黄) 2 胜 1 平 11 负，场均 demo 3.5 : 我们 0.8")
    print("  - rlg 仅用于运动学（球速/衰减/反弹）与球位分布参考（每场只录部分时段，进球不全）")
